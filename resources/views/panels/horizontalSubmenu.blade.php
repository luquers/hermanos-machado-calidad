...
<ul class="dropdown-menu">
  ...
  <li
    class="{{ $custom_classes }}{{ (isset($submenu['submenu'])) ? 'dropdown dropdown-submenu' : '' }} {{ $submenu['url'] === request()->url() ? 'sidebar-group-active active' : '' }}" @if(isset($submenu['submenu'])){{'data-menu=dropdown-submenu'}}@endif>
    <a href="{{isset($submenu['url']) ? url($submenu['url']):'javascript:void(0)'}}" class="dropdown-item {{ (isset($submenu['submenu'])) ? 'dropdown-toggle' : '' }} d-flex align-items-center"
      ...
      @endif
      <span>{{ __('menu.'.$submenu['name']) }}</span>
    </a>
